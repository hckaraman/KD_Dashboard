...
basins <- readOGR("./Data/HES_b.geojson")
hes <- readOGR("./Data/HES_p.geojson")
...
shinyServer(function(input, output) {
  ...
  observeEvent(input$hpp, {
    ...
    leafletProxy("mymap",data = temp_hes) %>% addMarkers(layerId = "foop") %>% clearMarkers() %>% addMarkers(label = ~popup)
